# fix-1e84b7b056f996d6 — Empty-favorites message references non-existent /addfav command

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`src/handlers/favs.ts:50` tells the user: `"Add some with /addfav <from>:<to>"`. The actual add command is `/fav` (from `E2T1.ts`). Trying `/addfav` will hit the bot's unknown-message fallback, giving the user a dead end.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-1e84b7b056f996d6.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-1e84b7b056f996d6.json` (and any new command `tests/commands/fix-1e84b7b056f996d6.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-1e84b7b056f996d6.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-1e84b7b056f996d6.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

# fix-6612f86cb0e7f5d2 — T03.ts /help command is dead code — shadowed by T01.ts

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

Both `src/handlers/T01.ts:6` and `src/handlers/T03.ts:6` register `composer.command("help", ...)`. Since `T01.ts` sorts before `T03.ts`, T01's `/help` handler always wins. T03.ts is entirely dead code. Both `tests/commands/T01.json` and `tests/commands/T03.json` declare `["/help"]` but the T01 handler covers both specs.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-6612f86cb0e7f5d2.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-6612f86cb0e7f5d2.json` (and any new command `tests/commands/fix-6612f86cb0e7f5d2.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-6612f86cb0e7f5d2.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-6612f86cb0e7f5d2.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

# fix-9d2d1cfebf818d18 — Test spec favs.json uses non-existent /addfav command to seed data

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`tests/specs/favs.json:6-16` seeds Redis data by sending `/addfav kg:lb` and `/addfav c:f`, expecting responses like `"Favorite added: kg:lb"`. No `/addfav` command handler exists anywhere in the codebase (the save command is `/fav`, implemented in `E2T1.ts`, which responds with `"Favorite saved: …"`). The unknown-message fallback from `buildBot()` will intercept `/addfav` and reply `"Sorry, I didn't understand that. Try /help."`, failing both seed steps. As a result, the subsequent `/favs` step will also fail because no data was seeded — it will get the empty-state message instead of the expected `"Your favorites:\nc:f\nkg:lb"`.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-9d2d1cfebf818d18.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-9d2d1cfebf818d18.json` (and any new command `tests/commands/fix-9d2d1cfebf818d18.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-9d2d1cfebf818d18.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-9d2d1cfebf818d18.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

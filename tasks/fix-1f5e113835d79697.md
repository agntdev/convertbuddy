# fix-1f5e113835d79697 — Redis key prefix mismatch: /fav and /favs use different namespaces

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`src/handlers/E2T1.ts:6` stores favorites under the key prefix `"convertbuddy:favs:"` (matching the E2T1 task spec: `convertbuddy:favs:{telegram_user_id}`). `src/handlers/favs.ts:5` reads from a **different** prefix: `"favs:"`. These are two completely separate Redis keyspaces — `/favs` will *never* return any favorites saved by `/fav`. The retrieval handler must use the identical prefix as the storage handler.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-1f5e113835d79697.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-1f5e113835d79697.json` (and any new command `tests/commands/fix-1f5e113835d79697.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-1f5e113835d79697.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-1f5e113835d79697.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

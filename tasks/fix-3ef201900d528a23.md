# fix-3ef201900d528a23 — Dead callback buttons: menu:convert and menu:help have no handler

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

The `/start` menu keyboard (src/handlers/start.ts:8-14) emits three inline buttons: `menu:convert`, `menu:favs`, `menu:help`. Only `menu:favs` has a `callbackQuery` handler (in src/handlers/E3T1.ts:151). Tapping **Convert** or **Help** produces an unanswered callback query — Telegram shows a perpetual loading spinner on the button and the user sees nothing happen. The `menu:convert` and `menu:help` callbacks are completely unhandled.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3ef201900d528a23.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3ef201900d528a23.json` (and any new command `tests/commands/fix-3ef201900d528a23.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3ef201900d528a23.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3ef201900d528a23.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

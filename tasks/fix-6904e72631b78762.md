# fix-6904e72631b78762 — Duplicate command declarations in tests/commands/

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`tests/commands/T01.json` and `tests/commands/T03.json` both declare `["/help"]`. `tests/commands/start.json` and `tests/commands/T02.json` both declare `["/start"]`. `tests/commands/favs.json` and `tests/commands/E3T1.json` both declare `["/favs"]`. These duplicates inflate the declared command list for coverage computation (the `normalizeDeclaredCommands` function deduplicates, so they collapse, but the duplication masks that `T03` and `favs` handlers are dead code).

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-6904e72631b78762.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-6904e72631b78762.json` (and any new command `tests/commands/fix-6904e72631b78762.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-6904e72631b78762.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-6904e72631b78762.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

# fix-87ffd5c8a1b3c22e — favs.ts /favs command is dead code — shadowed by E3T1.ts

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

Both `src/handlers/favs.ts:32` and `src/handlers/E3T1.ts:122` register `composer.command("favs", ...)`. `buildBot()` loads handlers in sorted filename order, so `E3T1.ts` is registered first and its `/favs` handler **always** wins. The entire `favs.ts` handler (its `/favs` command, its own `getRedis()` factory, its `FAV_KEY_PREFIX`) is unreachable dead code. The test specs `favs.json` and `E3T1.json` both pass because E3T1's handler satisfies both.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-87ffd5c8a1b3c22e.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-87ffd5c8a1b3c22e.json` (and any new command `tests/commands/fix-87ffd5c8a1b3c22e.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-87ffd5c8a1b3c22e.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-87ffd5c8a1b3c22e.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

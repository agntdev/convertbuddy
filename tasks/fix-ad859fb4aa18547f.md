# fix-ad859fb4aa18547f — Redis client created by E3T1 is never disconnected on shutdown

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`getRedis()` in `src/handlers/E3T1.ts:19-32` caches an ioredis client at module scope and never provides a way to disconnect it. On graceful shutdown (SIGTERM/SIGINT), the connection leaks. Note that this is a systemic pattern also present in `src/toolkit/session/redis.ts` and `src/handlers/E2T1.ts`, but each handler module creates its **own** separate ioredis connection to the same Redis server, compounding the issue.

**Impact:** Resource leak on bot restart/redeploy. In containerized environments this is typically mitigated by process termination, but graceful shutdown handlers should clean up open connections.

**Fix:** Either export a `disconnect()` function that the bot lifecycle can call, or share a single Redis client across handler modules.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-ad859fb4aa18547f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-ad859fb4aa18547f.json` (and any new command `tests/commands/fix-ad859fb4aa18547f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-ad859fb4aa18547f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-ad859fb4aa18547f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

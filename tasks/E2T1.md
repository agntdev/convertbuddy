# E2T1 — /fav command with Redis storage

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

Implement the /fav command handler in src/handlers/fav.ts. Validate unit pairs, store them in Redis under the pattern 'convertbuddy:favs:{telegram_user_id}', and handle duplicate/idempotent saves. Write tests to tests/specs/fav.json and register the command in tests/commands/fav.json.

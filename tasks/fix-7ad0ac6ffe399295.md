# fix-7ad0ac6ffe399295 — E2T1 /fav has no favorites-per-user limit check

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CVB

`src/handlers/E2T1.ts:33-98` — the `/fav` handler calls `sadd` without checking `scard` first. The spec (`docs/spec.md`) specifies a cap of 100 favorites per user. `E3T1.ts:66-72` enforces a limit (though it uses `MAX_FAVORITES = 50`, not the spec's 100). `E2T1.ts` has no limit at all — unbounded Redis growth.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-7ad0ac6ffe399295.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-7ad0ac6ffe399295.json` (and any new command `tests/commands/fix-7ad0ac6ffe399295.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-7ad0ac6ffe399295.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-7ad0ac6ffe399295.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
